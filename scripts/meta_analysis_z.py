...
def z_neff_meta( studies : List[Tuple['Study','VariantData']], is_het_test = False):
...
def z_ivw_meta(studies: List[Tuple['Study', 'VariantData']], is_het_test=False):
    # Z = sum(w_i * Z_i) / sqrt( sum(w_i^2) ), w_i = 1/se
    ...
    for s in studies:
        ...
        sum_weights += 1/dat.se**2
        sum_z += (1/dat.se) * dat.z
        tot_size += study.effective_size
...
def comb_meta( studies : List[Tuple['Study','VariantData']], is_het_test = False ):
    # meta-analyse non imputed Z-score with IVW, imputed with Neff, combine the two weighting for Neff

    # create vector of non-imputed and imputed studies
    ...
    for s in studies:
        study = s[0]
        dat = s[1]
        # raiss.imputed needs to be the first column

        if int(dat.extra_cols[0]) == 0:
            no_imp.append(s)
            cases_no_imp += study.n_cases
            controls_no_imp += study.n_controls
        else:
            imp.append(s)
            cases_imp += study.n_cases
            controls_imp += study.n_controls
...
SUPPORTED_METHODS = {"z_neff": z_neff_meta, "z_ivw": z_ivw_meta, "comb": comb_meta}


def check_eff_field(field):
    if field.lower() in ["z","or","z"]:
        return field.lower()
    else:
        raise Exception("effect_type must be z, OR, Z")
...
class VariantData:
    ...
    def __init__(self, chr, pos, ref, alt, z, pval, se=None, extra_cols=[]):
        self.chr = chr
    ...
    def __str__(self):
        return "chr:{} pos:{} ref:{} alt:{} z:{} pval:{} se:{}".format(self.chr, self.pos, self.ref, self.alt, self.z, self.pval, self.se)
...
def run():
    ...
    with open( outfile, 'w' ) as out:
        ...
        out.write("\t".join(["#CHR","POS","REF","ALT","SNP", studs[0].name + "_z", studs[0].name + "_se", studs[0].name + "_pval"  ]))
        ...
        for oth in studs[1:len(studs)]:
            out.write( "\t" +  "\t".join( [ oth.name + "_z", oth.name + "_se", oth.name + "_pval"] ))
            out.write( ("\t" if len(oth.extra_cols) else "") + "\t".join( [oth.name + "_" + c for c in oth.extra_cols] ) )

            if args.pairwise_with_first:
                for m in methods:
                    out.write("\t" + studs[0].name + "_" + oth.name + "_" +  m + "_meta_z\t" + studs[0].name + "_" + oth.name + "_" +  m + "_meta_se\t" + studs[0].name + "_" + oth.name + "_" +  m + "_meta_p")

        out.write("\tall_meta_N")
        for m in methods:
            if args.is_het_test:
                out.write("\tall_"+m+"_meta_z\tall_"+m+"_meta_se\tall_"+  m +"_meta_p\tall_"+ m +"_het_p")
            else:
                out.write("\tall_"+m+"_meta_z\tall_"+m+"_meta_se\tall_"+  m +"_meta_p")

        if args.leave_one_out:
            for s in studs:
                out.write("\t" + "leave_" + s.name + "_N")
                for m in methods:
                    out.write( "\t" +  "\t".join( ["leave_" + s.name + "_" + m + "_meta_z", "leave_" + s.name + "_" + m + "_meta_se", "leave_" + s.name + "_" + m + "_meta_p", "leave_" + s.name + "_" + m + "_meta_het_p"] ))
        ...
        while len(matching_studies)>0:
            ...
            for i,_ in enumerate(studs):
                if next_var[i] is not None:
                    outdat.extend([format_num(next_var[i].z), format_num(next_var[i].se), format_num(next_var[i].pval, 3)])
            ...
            if len( matching_studies )>1:
                ...
                meta_res.extend([format_num(matching_studies[0][1].z), format_num(matching_studies[0][1].se) , format_num(matching_studies[0][1].pval, 3), 'NA'] * len(methods))
            ...
            if args.leave_one_out:
                for s,_ in enumerate(studs):
                    ...
                    if len(matching_studies_loo) > 1:
                        ...
                        outdat.extend([format_num(matching_studies_loo[0][1].z), format_num(matching_studies_loo[0][1].se) , format_num(matching_studies_loo[0][1].pval, 3), 'NA'] * len(methods))
                    else:
