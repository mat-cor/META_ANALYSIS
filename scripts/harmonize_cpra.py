...
def harmonize(file_in, file_ref, n_total, require_gnomad, passing_only, gnomad_min_an):
    ...
    with gzip.open(file_in, 'rt') as f:
        ...
        has_n = 'N' in h_idx
        print('#CHR\tPOS\tSNPID\tAllele1\tAllele2\tAF_Allele2\timputationInfo\tBETA\tSE\tp.value\tAF_' + file_ref.replace('.gz', '') + '\tAF_fc\tN')
        for line in f:
            s = line.strip().split('\t')
            ...
            if not require_gnomad or len(equal) > 0:
                cpra = ':'.join([var.chr, str(var.pos), var.ref, var.alt])
                print('\t'.join([var.chr, str(var.pos), cpra, var.ref, var.alt,
